...
- 背景知识
  ...
  - Karma 是一个测试运行器，可以打开浏览器，加载测试脚本，然后运行测试用例
...
- 什么是持续集成
  ...
  持续集成是指代码只要有变更，就自动运行构建和测试，反馈运行结果。确保符合预期后，再将新代码集成到主干。持续集成的好处在于，每次代码的小幅变更，就能看到运行结果，从而不断积累小的变更，而不是在开发周期结束时，一下子合并一大块代码。
